...
int armOne = 0;     
int armTwo = 100;     
int slowforward = 1640;
int forward = 1675;
int reverse = 1325;
int stationary = 0;
const long ultraUpper = 225;
const long ultraLower = 180;
const long distanceUpper = 1000;
const long distanceLower = 300;
...
// SENSORS 
const int hallEffect = A0;
const int IRdistance = A1;
...
void setup() {
  ...
  // DRIVE MOTORS SETUP ATTACHED WHOLE TIME
...
/*******************************************/
// STEP #0
// INITIAL STEP TO GET THINGS SET UP, DOES NOT COUNT TOWARDS CHARLIEPLEX IF START FROM COUNTER = 0
if (counter == 0) {
  stepComplete = 1; 
...
// STEP#1 $$$
// THE CODE WILL BEGIN WHEN THE ROBOT IS WITHIN A CERTAIN DISTANCE OF THE WALL
// CHARLIEPLEX LIGHT WILL TURN ON WHEN THE ROBOT IS WITHIN THE DISTANCE
// CODE CONTINUES AFTER A 2 SECOND DELAY
if (counter == 1) {
  if (ul_Echo_Time > ultraLower && ul_Echo_Time < ultraUpper) {
    CharliePlexM::Write(11,1);
    startTime();
  }
  if (millis() - timer >= 2000 && ul_Echo_Time > ultraLower && ul_Echo_Time < ultraUpper) { 
    stepComplete = 1;
    CharliePlexM::Write(11,0);
  }
}

// STEP#2 $$$
// DRIVE ALONG THE WALL WITH CLAW OPEN
if (counter == 2) {          
...
// STEP#3 $$$
// CLAW CLOSE, WAIT 1 SECOND AND TURN PLATFORM
if (counter == 3) {
  startTime();
  servo_Claw.write(clawClosed);                                                                       
...
// STEP#4 $$$
// MOVE AWAY FROM WALL BRIEFLY BEFORE INITIATE FINDING PYRAMID
// DETACH SERVO_PLATFORM
// 90 DEGREE RIGHT TURN
// FORWARD TO GET AWAY FROM WALL
if (counter == 4) { 
  servo_Platform.detach();            
  ...
  if (millis() - timer > 2500 && millis() - timer < 3200) {
    forwardDrive();
  }
...
// STEP#5 $$$
// 2ND LINE OF CHARLIEPLEX
// INITIATE FINDING PYRAMID
// FORWARD WHEN IT SEES PYRAMID
// SCAN LEFT AND RIGHT, RIGHT 2 SECONDS, LEFT 4 SECONDS, RIGHT 6 SECONDS ETC....
if (counter == 5) { 
  if (Serial.read() == A || Serial.read() == E) { 
    forwardDrive();
    attempt = 1;
    timerCounter = 0; 
  }
...
// STEP#6 $$$
// PYRAMID WITHIN U OF FRAME, ADJUST PYRAMID QUICKLY
if (counter == 6) { 
  startTime();
  leftSlant();
  if (millis() - timer >= 600) {
...
// OPEN ARM, PYRAMID FALLS OVER CUBE
if (counter == 8) {
  startTime();
  servo_Claw.write(clawOpen);
  servo_Arm.write(armOne);
...
// LED 12 ALWAYS ON
// LEDS SWITCH HORIZONTALLY AFTER EACH STEP
// RESET STEP COMPLETE CHECK
// RESET TIMERS
// COUNTER++; NEXT STEP
void charlieplexCounter() {
  CharliePlexM::Write(12,1);
  CharliePlexM::Write(LED, 1);
  if (stepComplete == 1) {
    CharliePlexM::Write(LED, 0);
    LED = LED + 3;
    if (LED >= 13) {
      LED = LED - 14;
      if (LED < 1) {
