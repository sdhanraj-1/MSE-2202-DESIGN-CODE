...
if (counter == 1) {
  ...
  if (millis() - timer >= 2000 && ul_Echo_Time > ultraLower && ul_Echo_Time < ultraUpper) { // INITIAL SWITCH
    stepComplete = 1;
...
if (counter == 2) { // DRIVE ALONG THE WALL WITH CLAW OPEN                                         // IMPLEMENT ULTRASONIC SENSOR TO BE WITHIN A CERTAIN DISTANCE OF WALL.
  ...
  if (ul_Echo_Time < ultraLower) {
    ...
    servo_LeftMotor.writeMicroseconds(forward);                                                         // TEST SPEEDS
    servo_RightMotor.writeMicroseconds(forward);
  }
...
if (counter == 4) { // MOVE AWAY FROM WALL BRIEFLY BEFORE INITIATE FINDING PYRAMID
  ...
  if (millis() - timer <= 2500) { // TURN RIGHT X SECONDS, IDEALLY 90 DEGREE TURN                                          // TEST THIS MOVEMENT
    servo_LeftMotor.writeMicroseconds(forward);
    servo_RightMotor.writeMicroseconds(reverse);
  }
  if (millis() - timer > 2500 && millis() - timer < 4000) { // FORWARD TO GET AWAY FROM WALL
    servo_LeftMotor.writeMicroseconds(forward);
    servo_RightMotor.writeMicroseconds(forward);
  }
...
if (counter == 5) { // (2ND LINE OF CHARLIEPLEX) INITIATE FINDING PYRAMID
  ...
  /*
  if (step condition met) { IR DISTANCE SENSOR
    stepComplete = 1;
  } 
  */
}
if (counter == 6) { // PYRAMAID WITHIN U OF FRAME
  
}
if (counter == 7) { // PERFORM TIP AND DROP CUBE
  
}
if (counter == 8) { // DRIVE BACKWARDS
  if (timerCounter = 0) {
    timer = millis();
    timerCounter = 1;
  }   
